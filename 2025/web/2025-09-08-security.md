...
前端项目安全主要包括 XSS（跨站脚本攻击）、CSRF（跨站请求伪造）和点击劫持等。
...
跨站脚本攻击跟浏览器的[同源策略][same-origin_policy]有关，所谓同源策略，指的是两个站点进行通信时，他们的的**协议**、**域名**和**端口号**必需相同。只要三者中任意一个不同，就违背同源策略，也就是我们常说的“跨域”。比如，对于 `http://store.company.com/dir/page.html`，分析下面这几种情况。
...
同源策略的提出，主要是为了安全性的考虑。具体来说，同源策略有下面这些限制。

- 无法接触非同源网页的 DOM。
- 无法读取非同源网页的 Cookie、Storage 和 IndexedDB。
...
XSS 攻击通常分为三种类型：**存储型 XSS** (Stored XSS)、**反射型 XSS** (Reflected XSS)和**基于 DOM 的 XSS** (DOM-based XSS)。
...
反射型 XSS（Reflected XSS）是非持久化的攻击。恶意脚本不会存储在服务器上，而是通过 URL 参数或其他方式传递给服务器，然后立即反射回用户的浏览器。
...
输出编码包括了对输入的 HTML、URL 以及脚本等内容进行编码。下面是一个对各种字符编码的工具方法。
...
- JavaScript 和 CSS 上下文。将内容嵌入到 `<style>` 和 `<script>` 是一种危险的行为，应该避免这种情况的发生。
