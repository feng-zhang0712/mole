...
- **主题**（Subject）：被观察的对象，提供添加、删除和通知观察者的方法。
- **具体主题**（ConcreteSubject）：具体被观察的对象，维护观察者列表，状态改变时通知观察者。
- **观察者**（Observer）：观察主题的对象，定义更新接口，主题会调用观察者定义的接口，从而将数据传递给观察者。
- **具体观察者**（ConcreteObserver）：具体的观察者，实现更新接口。

观察者模式的优点是符合**开闭原则**，并且具有松耦合和可扩展的特点。缺点是：

- 如果观察者没有被正确移除，可能导致内存泄漏。
- 如果观察者和主题之间的关系处理不当，还可以导致循环依赖，进而导致无限递归。
- 观察者被通知的顺序也具有不确定性。

观察者模式适合多种场景，比如，事件绑定、消息推送、事件处理系统，以及一个对象的状态改变需要通知或改变其他对象时。
...
const useObservable = (initialValue) => {
  ...
  const notify = (newValue) => {
    setValue(newValue);
    observers.current.forEach(observer => observer(newValue));
  };
...
const ObservableComponent = () => {
  const { value, notify, subscribe } = useObservable(0);
  ...
  return (
    <div>
      <p>Value: {value}</p>
      <button onClick={() => notify(value + 1)}>Increment</button>
    </div>
  );
};
```
...
const ParentComponent: React.FC = () => {
  ...
  return (
    <div>
      ...
      <ChildComponent onUpdate={setData} />
    </div>
  );
};
...
发布-订阅模式有三个主要角色：发布者、订阅者和事件中心，发布-订阅模式的关键就在于事件中心。

- **发布者**（Publisher）：向事件中心发送事件或消息，发布者通常需要持有事件中心对象。
- **订阅者**（Subscriber）：向事件中心注册感兴趣的事件，处理接收到的消息以及取消订阅，订阅者通常也需要持有事件中心对象。
...
const pub = new ConcretePublisher(eventBus);

const sub1 = new ConcreteSubscriber(eventBus);
const sub2 = new ConcreteSubscriber(eventBus);

sub1.subscribe(EventTypes.USER_LOGIN, (data: UserLoginData) => {
  console.log('ConcreteSubscriber 1: User logged in', data);
});

sub2.subscribe(EventTypes.USER_LOGIN, (data: UserLoginData) => {
  console.log('ConcreteSubscriber 2: User logged in', data);
});
...
- 通信方式：在观察者模式中，主题和观察者直接通信；发布-订阅模式通过消息中心进行交互。
