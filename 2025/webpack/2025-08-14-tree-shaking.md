...
Tree Shaking 的核心是**静态分析**，这意味着在编译时（而不是运行时）就能确定模块的依赖关系。在传统的动态模块系统中，模块的依赖关系是在运行时确定的，构建工具无法在编译时知道哪些代码会被实际使用。

### 2.1 ESM 的静态特性

#### （1）顶层导入和导出

`import` 和 `export` 语句必须在模块的顶层，不能在条件语句或函数内部。顶层限制允许构建工具可以在编译时扫描所有 `import` 语句，这样模块的依赖关系在编译时就能完全确定。
...
导入路径必须是字符串字面量，不能是变量或表达式。这个特性确保了模块路径在编译时是确定的。
...
1. 首先检测 `package.json` 中的 `sideEffects` 属性，这个配置会覆盖所有其他的副作用检测逻辑。
   ...
   - 如果 `sideEffects` 是数组，则根据数组的配置，匹配文件是否有副作用。

2. 检测 webpack 配置中的 `sideEffects` 属性，如果属性为 `true`，表示启用副作用检测，此时构建工具会去获取 `package.json` 中的 `sideEffects` 配置。另外，在 `module.rules` 中可以为特定文件类型指定是否有副作用。
...
3. 启发式规则检测阶段。根据构建工具的内置规则，检测代码中是否有副作用。比如，上面分析的这些有副作用的代码，都会被显示或隐式地认为是有副作用的。
...
另外，如果构建工具在分析阶段遇到条件判断或者 `/*#__PURE__*/` 标记等场景，会单独进行处理，这些规则之间并不冲突，而是互补。
...
webpack 会分析每个模块的导出语句，追踪这些导出是否被其他模块实际使用。如果某个导出没有被使用，webpack 会将其标记为可移除的代码。这个分析过程基于静态分析
...
`minimize`：布尔值，控制是否启用代码压缩。生产模式下为`true`，其他模式为 `false`。
...
`CommonJS` 模块和ESM 模块的`import()` 动态导入，都属于运行时导入，无法在编译时确定模块的依赖关系，因此这两种方式完全不支持 Tree Shaking。所以，应该尽量避免下面的写法。
...
ESM 模块的默认导入和导出对 Tree Shaking 支持有限，因此会影响优化效果。
...
副作用是无处不在的，隐式的副作用、条件副作用、异步副作用、第三方库中的副作用...。因此，对副作用的识别是复杂的，但构建工具必须准确识别副作用，确保不会误删正常的代码。
...
上面的代码，即使 `sideEffects.js` 模块被标记有副作用，`console.log('Hello')` 依然被移除了。
