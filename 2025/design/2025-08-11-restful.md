...
RESTful（Representational State Transfer，表现层状态转移）是一种**软件架构风格**，由 [Roy Fielding] 在 2000 年提出。
...
所谓"上网"，就是与互联网上一系列的"资源"互动。

### 1.2 表现层（Representational）

**表现层**（Representation）指的是"资源"所表现出来的、外在的表现形式。

比如，文本可以用 txt、HTML、XML 或者 JSON 格式表现，甚至还可以采用二进制格式；图片可以用 JPG、PNG 等格式表现。
...
这种设计原则的优势是，能够提高系统的可扩展性，允许客户端和服务器独立开发和部署。
...
可缓存性（Cacheable）指响应必须明确标识是否可以被缓存，以及缓存的有效期。比如，HTTP 协议中，使用 `Expire`、`Cache-Control`、`Last-Modified` 和 `ETag` 等头字段控制缓存策略。
...
比如，HTTP 请求中，客户端通过 `Accept` 请求头指定接受数据的格式，服务器通过 `Content-Type` 响应头描述返回内容的格式。
...
比如，HTTP 请求中，请求由三部分构成：请求行、请求头和请求体。请求行中又包括请求方法、请求路径以及 HTTP 版本信息。在服务器返回的响应中，响应数据也包含三部分：响应行、响应头和响应体，响应行中又包含状态码、状态信息和 HTTP 版本信息。
...
通过链接来导航和发现资源，实现 API 的自发现。通过在响应中包含所需的链接，这样用户不用查询相关文档，即可使用相应的 API。

### 2.5 分层系统（Layered System）

客户端不能直接连接到最终服务器，必须通过中间层。客服端到服务器之间，还可以包含多个其他角色。比如，负载均衡服务器可以分发请求到多个服务器，反向代理能够处理 SSL、缓存、压缩，还可以设置监控层，实现系统监控和日志记录。
...
- 通过 `Accept` 头字段指定版本。优点是简洁，不会污染 URL 结构，缺点是需要在每个请求中都指定版本。
...
上面说过，实现资源的状态转移，要借助 HTTP 方法，下面介绍这些方法的使用方式。

#### （1）GET

`GET` 方法用来获取资源，该方法是**幂等**的，这意味着多次相同的请求，会得到相同的结果。`GET` 方法也是是安全的，一般不会改变资源的状态。
...
`POST` 方法用于创建新资源，该方法是**非幂等**的，这意味着多次相同的 `POST` 请求，可能会创建多个资源。`POST` 方法是不安全的，会改变资源的状态。
...
`PUT` 方法一般用于完整更新资源（替换整个资源），该方法是**幂等**的，多次相同的请求会得到相同的结果。`PUT` 方法也是安全的，一般不会改变资源的状态。
...
- `100 Continue`：有时，为了让服务器检查请求首部中的某些字符是否符合要求，客户端会发送一个请求，在请求头中有一个 `Expect: 100-continue` 字段，表示期望收到服务器返回的 `100 Continue` 状态码。如果服务器通过检索发现，请求头中的相关字段，满足服务器的要求，就会返回一个 `100 Continue` 状态码，表示客户端可以继续后续的请求；如果请求头中相关字段不满足服务器的要求，就会返回 `417 Expectation Failed` 状态码，表示相关字段不符合要求。
...
- `201 Created`：资源创建成功，且新增的资源已通过响应体返回。资源的地址，在响应体中返回，或者通过响应头中的 `Location` 字段指定。此状态码一般用于 `POST` 或者 `PUT` 请求成功时。
...
- `206 Partial Content`：请求成功，所请求的资源的部分内容已在响应中返回。通常该响应返回的数据区间，是通过请求头中的 `Range` 字段指定的，
  ...
  如果返回的响应中，只包含一个数据区间，那么响应头中的 `Content-Type` 字段的值为文件的类型；如果响应中包含多个数据区间，则整个响应头中 `Content-Type` 的值为 `multipart/byteranges`，每个数据片段会另外携带 `Content-Type` 和 `Content-Range` 字段，指明当前片段的数据类型和范围。
...
- `302 Found`：请求的资源被暂时移动到 `Location` 响应头字段所指向的新地址。浏览器在收到响应后，对 `Location` 指向的地址发送重定向请求，请求方法被修改为 `GET`。由于只是临时重定向，浏览器不会缓存重定向结果，搜索引擎也不会更改索引。
...
- `404 Not Found`：所请求的资源无法找到。该状态码只是单纯地说明资源没有找到，并没有说明资源是临时丢失还是永久丢失，如果资源是永久丢失，应该使用 `410 Gone`。
...
- `503 Service Unavailable`：服务器暂时不可用，一般是因为服务器处于维护状态。好的做法是，在返回 `503` 状态码的同时，响应体中给出具体的出错原因，并在头字段中通过 `Retry-After` 字段，告诉客户端多久之后可以尝试重新请求。
  ...
  上面代码表示，服务器当前不可用，客户端可以在 10/21/2015日7点28分之后或者 120s 后重新发送请求。
