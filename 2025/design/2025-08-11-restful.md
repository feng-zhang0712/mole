...
URL 版本控制有三种策略，可以分别通过指定为 URL 路径、查询参数的一部分，也可以使用头字段 `Accept`。
...
URI 中的 Endpoint 应该使用名词而非动词，并且，Endpoint 应该使用复数形式而非单数形式。
...
```javascript
// 获取文章列表
GET /api/articles?category=tech&page=1
...
POST /api/articles
{
  ...
  "content": "这是一篇关于 RESTful API 设计的文章...",
  "category": "tech"
}
