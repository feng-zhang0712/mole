...
一种是 `scroll(xCoord, yCoord)`，其中 `xCoord` 指定文档在水平轴移动的距离；`yCoord` 指定文档在垂直轴移动的距离。这两个属性都是以文档的左上角为基准点进行滚动。
...
`window.scroll()` 是 `window.scrollTo()` 的别名，`window.scrollTo()` 是 W3C 规范中定义的正式方法，语义更明确，表示“滚动到”特定位置。所以，推荐使用 `window.scrollTo()`，因为它更符合规范且更常出现在文档中。
...
`Element.scrollTo()` 方法跟 `window.scroll()` 的作用和使用方式完全一致，调用这个方法的对象，是具体的某个元素对象。

#### 3.5 `Element.scrollBy`

`Element.scrollBy()` 方法跟 `window.scrollBy()` 的作用和使用方式完全一致，调用这个方法的对象，是具体的某个元素对象。
