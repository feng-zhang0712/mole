...
- 跨浏览器兼容性。合成事件能够处理过时的事件 API，同时为浏览器不支持的 API 提供了支持，统一了不同浏览器事件之间的差异。
- 性能优化。事件委托减少了内存占用，批量更新避免了频繁渲染。
- 开发体验提升。合成事件提供了统一的 API 接口。

## 合成事件

React 采用**抽象工厂模式**来创建不同类型的合成事件。
...
`listenToAllSupportedEvents` 用于监听所有支持的原生事件，这里也是事件委托机制的实现部分。
...
// 下面这些事件不冒泡，所以将他们绑定在真实 DOM 节点上（非委托事件处理）
