...
优点：能够实现对父类（下边例子是 person 对象）中属性和方法的继承。
缺点：父类对象中的引用属性会被所有字类实例共享。

这种实现方式跟使用 `Object.create()` 方式效果一样。将下边的 `object()` 函数替换为 `Object.create()` 会得到同样的效果。
...
/*
每次执行 object() 方法，传入的是同一个 person 对象，这意味着 person 对象中的引用类型属性会被所有实例共享，
